...
public class Wolf : MonoBehaviour
{
    ...
    public GameObject dieItem;//死亡掉落物品
    //被淨化
    int mySour;//目前靈魂
    public GameObject cleItem;//淨化掉落物
    bool clear = false;//淨化(擊暈)用。預設為false，被淨化了才會變true
    //巡邏AI
    public float movePos;//移動座標，由編輯器指定
    float startPos; //起始座標
    float endPos; //結束座標
    float moveSpeed = 0.6f;//移動速度
    bool moveRight = true;//是否往右移動
                          //攻擊AI
    float lastTime = 0.0f;
    float waitTime = 1.2f;//攻擊間隔時間
    ...
    void Awake()
    {
        ...
        this.GetComponent<SpriteRenderer>().color = new Color32(146,250,255,255);
    ...
    void IfClear()        //是否淨化結束
    {
        if (mySour <= 0 && myHp <= 80 && !clear && !spec)
        {
            body.AddForce(new Vector2(0f, 40f));//往上飛
            StartCoroutine(Cleared());
            return;
        }
        if (mySour <= 0 && myHp <= 50 && !clear && spec)
        {
            body.AddForce(new Vector2(0f, 40f));//往上飛
            StartCoroutine(Cleared());
            return;
    ...
    void MonsWarnState()//警戒模式
    {
        ...
        {
            anim.SetBool("warn", true);
            transform.position = (Vector2.MoveTowards(transform.position, player.transform.position, moveSpeed * Time.deltaTime*3));
            //依畫幀更新時間和巡邏速度朝主角前進
    ...
    void MonsAtk()
    {
        ...
        {
            if (facingRight)
                body.AddForce(new Vector2(250f, 0f));
            else
                body.AddForce(new Vector2(-250f, 0f));
            anim.SetTrigger("atk");
        }
    ...
    IEnumerator Die()
    {
        ...
        GameObject.Instantiate(dieItem, transform.position + new Vector3(0, 1), Quaternion.identity);
        if (spec)
        {
            GameObject.Instantiate(dieItem, transform.position + new Vector3(0, 1), Quaternion.identity);
            GameObject.Instantiate(dieItem, transform.position + new Vector3(0, 1), Quaternion.identity);
    ...
    IEnumerator Cleared()
    {
        ...
        this.gameObject.layer = 14;//機關用，設定層為物品
    }
